"""
3D UNet for conditional (guided) diffusion models.
"""
...
class UNet3D(nn.Module):
    """
    The full 3D UNet model with attention and embedding for conditional diffusion.
    ...
    :param attn_res: a collection of downsample rates at which attention will take place.
    ...
    :param num_heads_channels: if specified, ignore num_heads and use fixed channel width per head.
            ...
            for i in range(res_blocks + 1):
                ...
                if level and i == res_blocks:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else Upsample3D(ch, conv_resample, out_channel=out_ch)
                    )
                    ds //= 2
    ...
    def forward(self, x, gammas):
        ...
        :param x: an [N x C x D x H x W] Tensor of inputs (3D volumes)
